...
/**
 * Created by Benjamin DANGLOT benjamin.danglot@inria.fr on 18/12/17
 */
public abstract class JacocoRunner {
	...
	/**
	 * @param classesDirectory     the path to the directory that contains the
	 *                             .class file of sources
	 * @param testClassesDirectory the path to the directory that contains the
	 *                             .class file of test sources
	 */
	...
	/**
	 * Compute the instruction coverage of the given tests Using directly this
	 * method is discouraged, since it won't avoid class loading conflict. Use
	 * {@link EntryPoint#runCoverage(String, String, String[], String[])} instead.
	 *
	 * @param classesDirectory             the path to the directory that contains
	 *                                     the .class file of sources
	 * @param testClassesDirectory         the path to the directory that contains
	 *                                     the .class file of test sources
	 * @param fullQualifiedNameOfTestClass the full qualified name of the test class
	 *                                     to execute
	 * @param testMethodNames              the simple names of the test methods to
	 *                                     exeecute
	...
	public Coverage run(String classpath, String classesDirectory, String testClassesDirectory,
		...
		try {
			...
			// this.instrumentedClassLoader.addDefinition(fullQualifiedNameOfTestClass,
			// IOUtils.toByteArray(classLoader.getResourceAsStream(resource)));

			runtime.startup(data);
			final Coverage listener = this.executeTest(new String[] { fullQualifiedNameOfTestClass }, testMethodNames,
	 ...
	 * @param testClassesDirectory           the path to the directory that contains
	 *                                       the .class file of test sources
	 * @param fullQualifiedNameOfTestClasses the full qualified names of the test
	 *                                       classes to execute
	 * @return a {@link Coverage} instance that contains the instruction coverage of
	 *         the given tests.
	 */
	public Coverage run(String classesDirectory, String testClassesDirectory,
		...
		Arrays.stream(fullQualifiedNameOfTestClasses).forEach(fullQualifiedNameOfTestClass -> {
			final String resource = ConstantsHelper.fullQualifiedNameToPath.apply(fullQualifiedNameOfTestClass)
					+ ".class";
			// MM: avoid register twice
			// try {
			// this.instrumentedClassLoader.addDefinition(fullQualifiedNameOfTestClass,
			// IOUtils.toByteArray(classLoader.getResourceAsStream(resource)));
			// } catch (IOException e) {
	...
	private void instrumentAll(String classesDirectory) {
		final Iterator<File> iterator = FileUtils.iterateFiles(new File(classesDirectory), new String[] { "class" },
				true);
		while (iterator.hasNext()) {
			final File next = iterator.next();
			final String fileName = next.getPath().substring(
					classesDirectory.length() + (classesDirectory.endsWith(ConstantsHelper.FILE_SEPARATOR) ? 0 : 1));
			final String fullQualifiedName = ConstantsHelper.pathToFullQualifiedName.apply(fileName).substring(0,
