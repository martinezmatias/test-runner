...
/**
 * created by Benjamin DANGLOT benjamin.danglot@inria.fr on 11/04/19
 */
public class JUnit5JacocoRunner extends JacocoRunner {
	...
	/**
	 * The entry method to execute junit tests. This method is not meant to be used
	 * directly, but rather using {@link EntryPoint} For the expected arguments, see
	 * {@link ParserOptions}
	 */
	public static void main(String[] args) {
		final ParserOptions options = ParserOptions.parse(args);
		final String[] splittedArgs0 = options.getPathToCompiledClassesOfTheProject()
				.split(ConstantsHelper.PATH_SEPARATOR);
		final String classesDirectory = splittedArgs0[0];
		final String testClassesDirectory = splittedArgs0[1];
		final JacocoRunner jacocoRunner = new JUnit5JacocoRunner(classesDirectory, testClassesDirectory,
				options.getBlackList());
		final String[] testClassesToRun = options.getFullQualifiedNameOfTestClassesToRun();
		if (testClassesToRun.length > 1) {
			jacocoRunner.run(classesDirectory, testClassesDirectory, testClassesToRun).save();
		} else {
			if (options.getTestMethodNamesToRun().length == 0) {
				jacocoRunner.run(classesDirectory, testClassesDirectory, testClassesToRun).save();
			} else {
				// TODO MM
				jacocoRunner.run("", classesDirectory, testClassesDirectory, testClassesToRun[0],
						options.getTestMethodNamesToRun()).save();
			}
		}
	}
	...
	protected Coverage executeTest(String[] testClassNames, String[] testMethodNames, List<String> blackList) {
		...
		JUnit5Runner.run(testClassNames, testMethodNames, blackList, listener, this.instrumentedClassLoader);
		return listener;
	}
}
