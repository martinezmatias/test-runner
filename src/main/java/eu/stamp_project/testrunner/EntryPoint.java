...
public class EntryPoint {
	...
	/**
	 * This parameter allows to configure the mutation engine you want to use. By
	 * default, it uses descartes (c.f.
	 * https://github.com/STAMP-project/pitest-descartes) Possible values:
	 * ConstantsHelper.mutationEngine.DESCARTES or
	 * ConstantsHelper.mutationEngine.GREGOR see
	 * {@link eu.stamp_project.testrunner.utils.ConstantsHelper.MutationEngine}
	 */
	private static ConstantsHelper.MutationEngine mutationEngine = ConstantsHelper.MutationEngine.DESCARTES;

	/**
	 * This method configure the mutation engine to be used. It also sets the
	 * default values for the mutators, see
	 * {@link eu.stamp_project.testrunner.EntryPoint#pitMutators}
	 *
	 * @param mutationEngine the mutation engine to be used. see
	 *                       {@link eu.stamp_project.testrunner.utils.ConstantsHelper.MutationEngine}
	 */
	...
	/**
	 * List of mutators to be used. By default, it uses the default mutators of
	 * descartes. If you specify the gregor mutation engine, it uses ALL mutators
	 * (c.f. http://pitest.org/)
	 */
	public static List<String> pitMutators = mutationEngine.mutators;

	/**
	 * Configure either using XML format for the output or CSV. See
	 * {@link eu.stamp_project.testrunner.listener.pit.AbstractParser.OutputFormat}
	 */
	 ...
	 * </p>
	 *
	 * @param classpath                      the classpath required to run the given
	 *                                       test.
	 ...
	 * @param classpath                      the classpath required to run the given
	 *                                       tests classes.
	 * @param targetProjectClasses           path to the folder that contains
	 *                                       binaries, i.e. .class, on which Jacoco
	...
	public static Coverage runCoverage(String classpath, String targetProjectClasses,
		...
		final String javaCommand = String.join(ConstantsHelper.WHITE_SPACE,
				new String[] { getJavaCommand(),
						...
						EntryPoint.jUnit5Mode ? EntryPoint.JUNIT5_JACOCO_RUNNER_QUALIFIED_NAME
								: EntryPoint.JUNIT4_JACOCO_RUNNER_QUALIFIED_NAME,
	...
	public static CoveragePerTestMethod runCoveragePerTestMethods(String classpath, String targetProjectClasses,
		...
		final String javaCommand = String.join(ConstantsHelper.WHITE_SPACE,
				new String[] { getJavaCommand(),
						classpath + ConstantsHelper.PATH_SEPARATOR + ABSOLUTE_PATH_TO_RUNNER_CLASSES
								+ ConstantsHelper.PATH_SEPARATOR + ABSOLUTE_PATH_TO_JACOCO_DEPENDENCIES,
						EntryPoint.jUnit5Mode ? EntryPoint.JUNIT5_JACOCO_RUNNER_PER_TEST_QUALIFIED_NAME
								: EntryPoint.JUNIT4_JACOCO_RUNNER_PER_TEST_QUALIFIED_NAME,
						ParserOptions.FLAG_pathToCompiledClassesOfTheProject, targetProjectClasses,
						ParserOptions.FLAG_fullQualifiedNameOfTestClassToRun,
						String.join(ConstantsHelper.PATH_SEPARATOR, fullQualifiedNameOfTestClasses),
						methodNames.length == 0 ? ""
	...
	/**
	 * @param classpath           the classpath of the project for which we need to
	 *                            compute the mutation score
	 * @param pathToRootProject   the path to the root folder of the project
	 * @param filterTargetClasses the regex to match source classes (application) to
	 *                            mutate
	 * @param targetTests         the full qualified name of the test class for
	 *                            which we want to compute the mutation score.
	 * @return a list of
	 *         {@link eu.stamp_project.testrunner.listener.pit.AbstractPitResult}
	 *         that contains the mutation analysis. The type depends on the format
	 *         used.
	 */
	public static List<? extends AbstractPitResult> runPit(final String classpath, final String pathToRootProject,
			final String filterTargetClasses, final String targetTests) {
		final String classpathToExecute;
		if (jUnit5Mode) {
			classpathToExecute = String.join(ConstantsHelper.PATH_SEPARATOR, new String[] { classpath,
					ABSOLUTE_PATH_TO_PIT_DEPENDENCIES, ABSOLUTE_PATH_TO_PIT_DEPENDENCIES_FOR_JUNIT5 });
		} else {
			classpathToExecute = String.join(ConstantsHelper.PATH_SEPARATOR,
					new String[] { classpath, ABSOLUTE_PATH_TO_PIT_DEPENDENCIES });
		}
		PitRunner.main(new String[] { classpathToExecute, pathToRootProject, filterTargetClasses, targetTests,
				EntryPoint.pitOutputFormat.name().toLowerCase(), EntryPoint.mutationEngine.name().toLowerCase(),
				String.join(ConstantsHelper.PATH_SEPARATOR, EntryPoint.pitMutators) });
		return AbstractParser.build(EntryPoint.pitOutputFormat)
				.parseAndDelete(pathToRootProject + PitRunner.REPORT_PITS);
	}
	...
	private static final Function<List<Class<?>>, String> CLASSES_TO_PATH_OF_DEPENDENCIES = classes -> classes.stream()
			...
			.map(EntryPoint::RemoveWinFileSeparator).peek(path -> LOGGER.info("{}", path))
	...
	private static final List<Class<?>> PIT_DEPENDENCIES = Arrays.asList(GregorMutationEngine.class,
			TestGroupConfig.class, PluginServices.class, DescartesMutationEngine.class);

	private static final List<Class<?>> PIT_DEPENDENCIES_FOR_JUNIT5 = Arrays.asList(JUnit5Configuration.class,
			ExtensionContext.class, TestAbortedException.class);
