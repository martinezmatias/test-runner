...
/**
 * created by Benjamin DANGLOT benjamin.danglot@inria.fr on 13/11/18
 */
public class TestResultImpl implements TestResult, Serializable {
	...
	public List<String> getPassingTests() {
		final List<String> failing = this.failingTests.stream().map(failure -> failure.testCaseName)
				.collect(Collectors.toList());
		final List<String> assumptionFailing = this.assumptionFailingTests.stream().map(failure -> failure.testCaseName)
				.collect(Collectors.toList());
		return this.runningTests.stream().filter(description -> !assumptionFailing.contains(description))
				.filter(description -> !failing.contains(description)).collect(Collectors.toList());
	}
	...
	public Failure getFailureOf(String testMethodName) {
		return this.getFailingTests().stream().filter(failure -> failure.testCaseName.equals(testMethodName))
				.findFirst().orElseThrow(() -> new IllegalArgumentException(
						String.format("Could not find %s in failing test", testMethodName)));
	}
	...
	public synchronized void save() {
		...
		System.out.println("File saved to the following path1: " + f.getAbsolutePath());
	}
	...
	public String toString() {
		return "TestResultImpl{" + "runningTests=" + this.getRunningTests() + ", failingTests=" + this.getFailingTests()
				+ ", assumptionFailingTests=" + this.getAssumptionFailingTests() + ", ignoredTests="
				+ this.getIgnoredTests() + '}';
	}
}
