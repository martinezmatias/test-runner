...
public class CoverageImpl implements Coverage, Serializable {
	...
	public static List<Integer> getListOfCountForCounterFunction(IClassCoverage coverage,
		...
		return coverage.getMethods().stream().filter(iMethodCoverage -> !"<clinit>".equals(iMethodCoverage.getName()))
				.flatMap(iMethodCoverage -> IntStream
						.range(iMethodCoverage.getFirstLine(), iMethodCoverage.getLastLine() + 1)
						.mapToObj(iMethodCoverage::getLine).map(ILine::getInstructionCounter).map(counterGetter))
	...
	public void collectData(ExecutionDataStore executionData, String classesDirectory) {
		...
		coverageBuilder.getClasses().forEach(coverage -> {
			final List<Integer> listOfCountForCounterFunction = CoverageImpl.getListOfCountForCounterFunction(coverage,
					ICounter::getCoveredCount);
			builderExecutionPath.append(coverage.getName()).append(":").append(
					listOfCountForCounterFunction.stream().map(Objects::toString).collect(Collectors.joining(",")))
					.append(";");
			counter[0] += listOfCountForCounterFunction.stream().mapToInt(Integer::intValue).sum();
			counter[1] += CoverageImpl.getListOfCountForCounterFunction(coverage, ICounter::getTotalCount).stream()
